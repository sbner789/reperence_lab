...
const useMarketGuideViewer = (dataUrl: string) => {
  ...
  const [marketGuide, setMarketGuide] = useState<MarketGuideMeta[]>([]);

  useEffect(() => {
    const fetchGuideData = async () => {
      const [ section_1, section_2 ] = await Promise.all([
        axios.get("/local_data/market_section_3-1.json"),
        axios.get("/local_data/market_section_3-2.json")
      ])

      const sectionMeta_1 = section_1.data.section;
      const sectionMeta_2 = section_2.data.section;

      setMarketGuide([sectionMeta_2]);
    }
